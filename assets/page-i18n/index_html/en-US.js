...
//@@zh="回声消除 AEC（一般默认打开）"
,"ujUi:"+ //no args
       "AEC (usually enabled by default)"

//@@zh="不设置"
,"3K14:"+ //no args
       "Not Set"

//@@zh="降噪 ANS（依赖AEC未显式关闭）"
,"Q0lg:"+ //no args
       "ANS (depends on AEC not explicitly disabled)"

//@@zh="不设置"
,"K3dr:"+ //no args
       "Not Set"

//@@zh="自动增益 AGC（依赖AEC未显式关闭）"
,"gjGg:"+ //no args
       "AGC (depends on AEC not explicitly disabled)"
...
//@@zh="；均为set中的audioTrackSet高级配置，会直接传递给浏览器的getUserMedia方法，不同浏览器的支持程度不同，并不一定会生效；这些参数不设置时浏览器一般默认打开，一般AEC未被显式禁用时ANS、AGC才会有效；"
,"7ots:"+ //no args
       ". These are all advanced configurations in the audioTrackSet class, which will be directly passed to the browser's getUserMedia method. Different browsers have varying levels of support, and they may not always take effect. Browsers generally enable these parameters by default when they are not set. ANS and AGC are usually only effective when AEC is not explicitly disabled. "

//@@zh="移动端打开降噪、回声消除可能会表现的很怪异（包括系统播放音量变小），关闭又可能导致录音音量变小或非常小（尤其是iOS）"
,"40Wa:"+ //no args
       "On mobile devices, turning on noise cancellation and echo cancellation can cause strange behavior (including a decrease in system playback volume), while turning them off can result in a decrease or even a very low recording volume (especially on iOS)"

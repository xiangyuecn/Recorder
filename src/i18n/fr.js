...
//@@zh="提示：未配置 {1} 时浏览器可能会自动启用回声消除（含降噪）；未禁用回声消除时可能会导致：open打开录音会很耗时（禁用后打开变快），移动端降低系统播放音量（关闭录音后可恢复）、录音音量很小或忽大忽小、和可能只提供16k采样率的音频流（禁用后获得48k高音质流）；请参阅文档中{2}配置"
//@@en="Note: If {1} is not configured, the browser may automatically enable echoCancellation (including noiseSuppression); if echoCancellation is not disabled, it may cause: opening the recording window will take a long time (disabling it will make opening faster), the system playback volume on mobile devices will decrease (this can be restored after closing the recording), the recording volume will be very low or fluctuate, and it may only provide an audio stream with a 16kHz sampleRate (disabling it will provide a 48kHz high-quality stream); please refer to the configuration in {2} of the documentation"
,"RiWe:"+ //args: {1}-{2}
       "Remarque: Si {1} n’est pas configuré, le navigateur peut activer automatiquement l’annulation d’écho (y compris la suppression du bruit). Si l’annulation d’écho n’est pas désactivée, cela peut entraîner les problèmes suivants: l’ouverture de la fenêtre d’enregistrement sera longue (la désactiver accélérera l’ouverture), le volume de lecture système sur les appareils mobiles diminuera (il sera rétabli après la fermeture de l’enregistrement), le volume d’enregistrement sera très faible ou fluctuant, et le flux audio pourrait n’avoir qu’une fréquence d’échantillonnage de 16kHz (la désactiver permettra d’obtenir un flux haute qualité de 48kHz). Veuillez vous référer à la configuration décrite dans {2} de la documentation"
...
//@@zh="提示：在onProcess中打印buffers等二进制数据、部分WebView在后台运行受限，均可能导致卡顿"
//@@en="Note: Printing binary data such as buffers in onProcess and having some WebViews run in the background with restrictions can both cause stuttering"
,"bM6i:"+ //no args
       "Remarque: L’affichage de données binaires telles que des tampons dans onProcess et l’exécution de certaines WebViews en arrière-plan avec des restrictions peuvent toutes deux provoquer des saccades"
...
//@@zh="非单或双声道wav raw pcm格式wav，不支持解码"
//@@en="Non-mono or stereo wav raw pcm format wav files are not supported for decoding"
,"jXp3:"+ //no args
       "Les fichiers WAV au format wav raw pcm non mono ou stéréo ne sont pas pris en charge pour le décodage"
